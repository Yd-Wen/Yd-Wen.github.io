...
- font：是否倾斜 是否加粗 字号/行高 字体 （一般设置公共文字样式，**必须按顺序书写**，**字号和字体必须书写**，否则 font 属性不生效）。

- text-indent：数字+em（1em 等于当前字号）。

- text-align：center （常用，用于设置内容水平居中。也可设置图片居中，用父容器包裹图片，设置父容器 text-align）。
